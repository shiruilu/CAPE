...
def ellipse(A, B, bound=1.0, prob=1.0):
    '''test a CIELab color falls in certain ellipse'''
    return (1.0*(A-143)/6.5)**2 + (1.0*(B-148)/12.0)**2 < (1.0*bound/prob)
...
def skin_detect(img):
    ...
    for (i,j), value in np.ndenumerate(skinMask):
        if skinMask[i,j] ==0:
            if ellipse_test(img_LAB[i,j][1], img_LAB[i,j][2], \
                                bound=1.25, prob=0.9):
                if check_neighbor(skinMask, i, j):
                    skinMask[i,j] = 255
                   # and check_neighbor(skinMask, i, j) \
                # else 0
    '''
    
    # for test check_neighbor same as check_hood
    test = np.zeros(skinMask.shape).astype(bool)
    for (i,j), value in np.ndenumerate(skinMask):
        if skinMask[i,j] == 0:
            if ellipse_test(img_LAB[i,j][1], img_LAB[i,j][2], bound=1.25, prob=0.9):
                if check_neighbor(skinMask, i, j):
                    test[i][j] = True

    test_hood = (skinMask==0) & check_hood(skinMask) & ellipse(img_lab[...,1], img_lab[...,2], bound=1.25, prob=0.9)
    print test == test_hood, (test==test_hood).all()
